...
- **The output window was black, and it was a STORAGE PARTITION** (v1.17.0, 2026-09-04). The window opened and closed correctly — the handle was real — but `Play` and `Show white` never arrived. **Measured, not guessed**: both documents report the same origin, a value written to `localStorage` in the control page reads `null` in the output window, and a `postMessage` on the handle arrives. **Chromium partitions storage by top-level site**, so a control page framed inside Pregonero is in a different partition from a top-level output window, and `BroadcastChannel` is storage. **Every message now goes out on both routes** — the channel for a window opened by hand standalone, the handle for the framed case — and each carries a `seq` the output applies once, so double delivery is harmless rather than avoided.
- **The Shape accordion's fields could not be typed in** (v1.17.0). **It was not the drag** — `draggable` was on the grip alone — it was **the row's own click**, which calls `selectShape`, which re-renders the list: clicking into a field destroyed the field mid-click. Stopped at the accordion boundary rather than on each input, because the accordion is the editor and the row is the selector, and that is one place rather than a rule to remember for every control added later.
- **The default shapes are named `Video frame`, `Video lyrics` and `Song lyrics`** (v1.17.0). `Frame` was ambiguous — the projector's frame or the video's — and the pair names itself now.
- **The visuals listing recurses and returns relative paths** (v1.17.0), because a real folder keeps its animations one level down in a per-song directory and a root-only listing offered a README and nothing else. **The constraint that mattered was *not absolute*, not *one segment***: `tragedia/pig.mov` leaks no more about where the folder lives than `pig.mov` does. Rooted, no traversal, symlinks neither followed nor listed, dotfiles dropped, bounded in depth and count — and **filtered to media on the listing only**, never on the fetch, because what is offered and what is served are different questions.

- **`song-intro` and `gig-contact` stopped being shape types** (v1.18.0, 2026-09-04). Two of the four song-aware types are gone from this tool: **Pregonero renders the intro card and the contact panel into a shape that already exists — the video frame or the lyrics shape — and decides which.** *Muralista owns where things are; Pregonero owns what is showing when*, and an intro is a when. **The photographs settled it**: on the canvas both read as neat rectangles of equal standing, and on the wall the intro landed on a whiteboard and the contact on bare planks, dim and low-contrast, because a shape that is up only when nothing else is still had to claim its own territory. This reverses two earlier decisions on purpose — 24/08 gave the intro its own shape so its text could size independently, which **the template's auto-fit already answers**, and an earlier pass this same day made both ordinary shapes in the default room, which lasted one walk. The template's proportions, its stand-in strings and the QR's file field went with them.
- **`Previewing:` drives the wall, not just the canvas** (v1.18.0). The toggle picked which face of a conditional pair the canvas drew, and **the output window painted both** — it had no conditions in it at all. The rule is written once now and evaluated in both windows, with the *answer* riding on the state message rather than the toggles, because the output has no toggles and no scope and should not have a second opinion.
- **The transport moved into the song** (v1.18.0). `Play`, `Pause` and `Restart` were three buttons in the flow bar that played **every video layer on the output at once**, on every screen including the deal and the photograph. They sit in the canvas band now, beside the song's name, **shown only when that song has a video with a file behind it** — which is what gives back playing a clip to see it in action. Changing songs pauses rather than carrying playback over.
- **The Backdrop fold opens with the page, and the shape you select opens with it** (v1.18.0). Both close when focus leaves them, by the same rule, so neither has to be found before it can be used and neither stays in the way. **And the selected shape is never drawn locked in Mode B**: *locked* answers *why can I not drag this*, which is only ever asked about the shape being pointed at.

- **Conditional visibility became NAMED MODES** (v1.19.0, 2026-09-05). Jorge named the two modes out loud — `Song with lyrics` and `Song with video and lyrics` — and **a name with nowhere to live is the symptom of a missing concept.** A mode is **a condition plus a set of shapes**, and one object replaced four things: the per-shape condition editor, the preview chip on `1 SHAPES`, the same chip owed on `2 OUTPUT`, and an unwritten assumption that the two conditions partition. **Nothing enforced that they were complements** — two independent rules can both be true or both be false, and the room then paints twice or not at all. **Exclusivity comes from the rule now:** modes are an ordered list, the first whose condition is true wins, and **when none matches no mode is live and only the no-mode shapes paint.**
- **A shape belongs to one mode or to none, and no mode means always displayed.** Membership is a field on the shape, so *exactly one* is structural rather than a rule to enforce and deleting a shape takes its grouping with it. **`visuals.json` gains `modes`, and the list is honest**: `mapper/modes.test.mjs` renders a hand-written **three-mode** room, because an array the renderer reads as `[0]` and `[1]` is a format promising what the code does not do. The authoring surface still seeds exactly two and has **no `+ Add mode`** — the door is real and stays shut until a second kind of condition exists.
- **The shape list is grouped, and the grouping IS the assignment surface.** Jorge's ruling: *dependencies are never set up from inside a shape*. A shape joins a mode by being **dragged into its group**, the gesture already on those rows for paint order. A mode is **renamed in place, and emptied rather than deleted** — deleting one orphans its shapes. Its **condition renders read-only**, so the concept is visible without shipping an editor for a language with one sentence in it. **The mode you are not previewing keeps its header and loses its rows**: he does not want to see the shapes of the mode he is not looking at, and the header is still where a shape is dropped into it.
- **The mode names are the preview selector, on `1 SHAPES` and on `2 OUTPUT`.** Active in green, which in this palette means *live on the wall right now* — pressing one broadcasts, so it is true. **`Show dependencies`, the per-shape `⇢ Frame filled` badge and the arrow overlay all went with the relation they drew.**
- **`2 OUTPUT`'s preview toggle did nothing, and this is why** (v1.19.0). Both states rendered the same picture. **Measured:** `renderPhotoOutlines` filtered on `shape.visible` and on nothing else — it never asked about conditions at all, so the chip above it was live, pressable, and could not change one outline. Closed **by verification**: the two modes now list different shapes on that screen.
- **A song's video was assigned and `Play` did not play it — three causes in a row** (v1.19.0), traced rather than guessed. **One:** `referencedMediaNames` collected `video` and `image` layer sources only, so a name a song assigned was never resolved to bytes and never broadcast. **Two:** hosted — framed inside Pregonero, where there is no directory handle — `syncResolvedMedia` had no branch at all and resolved nothing. **Three:** a `song-video` shape mounted a placeholder panel and no `<video>`, so there was nothing for the transport to play. None of them was a regression; all three were the state the file was left in when song visual setup shipped, and **the one thing nobody had done was play a song's clip in Muralista's own output window.** The comment saying *Muralista is never allowed to know which file that is* was defending a boundary that had already moved — this tool **authors** the assignment. What still holds is the line about songs: it reads `gig.json` and never a song's words, timeline or tempo.
- **The dummy lyrics come out of the canvas** (v1.19.0). Jorge retracts his own decision of the day before, and the reason is the one beside it: **the canvas shows a small offset between a shape and the whiteboard that is not there on the wall** — camera and projector do not share a viewpoint — and **placeholder text inside a shape sitting slightly off the real board reads as a misplacement rather than as a stand-in.** He positions against the wall, so **the canvas owes identification, not legibility**: outlines and labels stay, the words go. The stand-in still reaches the output window, which is where `maxSize` and `aspect` are tuned. **A `text` shape still draws its own string** — that is content somebody typed, not a fiction — and the canvas draws what is real.

- **`2 OUTPUT` has no song scope. It is always the whole room** (v1.20.0, 2026-09-05, Jorge). He expected the mode selector there and it was absent — and **the code was right**: the selector hides whenever a song is scoped, because the song's own assignments decide which mode is live and a selector the song overrules is a control that lies. **The defect was one level up: a song scope was live on OUTPUT at all**, persisting in from `1 SHAPES`, on a screen that is the photograph of the room. **Entering drops the scope**, so the selector needs no special case — and **it subsumes the transport finding**, because `Play / Pause / Restart` hangs off the song context bar that goes with it. Nothing was added to the screen: no picker, no per-song view, and **no read-only line naming the resolved mode**, which Cowork proposed and Jorge rejected as unnecessary once the scope cannot arrive.
- **The output window stays open** (v1.20.0). **This reverses P6d of 04/09**, which closed it on entering `2 OUTPUT`. His reason: *I'd still like to see it, it reassures me.* Reversed deliberately — `closeOutputWindow` is gone rather than left standing in a comment.
- **Shapes are editable from inside a song scope** (v1.20.0, Jorge). He wanted to reshape the video frame while previewing a song and had to go to `All`. **Editing a shape works in any scope and it edits the shape in the room, not for that song** — handles, name, type, format, opacity, the bin and `+ Add shape` all return, and **nothing draws locked any more**. The ruling underneath is untouched and is why it is safe: **there is still no per-song geometry.** The band's copy moved with the rule, from *Assignment only… the room is not edited here* — the old rule written down — to **shapes belong to the room; what goes in them belongs to the song**, which states the boundary rather than the behaviour on one side of it. `Set it in All` went too: it was the same wall one section over.
- **Media renders as media wherever a shape is shown** (v1.20.0, Jorge). A logo shape drew the words `🖼 image` on the canvas while the file was right there and resolved. **A text stand-in is never a substitute for media that exists** — the badge was written when previewing media was *out of scope for v1*, and the room now has always-on shapes with real content in them. It is **the output's own element factory** in a `foreignObject`, warped by the same matrix, never a second implementation; the badge survives only for shapes with nothing to draw.
- **A logo that would not load, traced in three parts** (v1.20.0). **The spaces-and-hyphen suspect is disproved, not deprioritised**: the hosted fetch encodes each segment and returns 200. What was true is that **the field's label lied under a mount** — `Source (relative to mapper/media/)` with a placeholder of `e.g. media/character.png`, both standalone-era strings, on a screen where names are looked up in the visuals folder's root — and that **hosted, a name that failed to resolve was reported nowhere**, because P6b hid the whole media section and the failure list went with it. So the label is true in whichever case is live, **a picker offers what the mount actually holds**, and the failures are shown even where the folder question is settled.
- **The selected shape is marked on the wall** (v1.20.0, Jorge): resizing against the room was done half blind while the border existed only in the preview. Editor chrome on the output — setup, not performance — drawn as an overlay in real output pixels, dark under bright so it survives a logo on white, and gone whenever nothing is selected.
- **Three smaller things** (v1.20.0). **Changing mode deselects a shape the new mode does not contain**, which was leaving ghost handles on a canvas that no longer drew the shape. **The expanded shape editor is indented with a rule down its left edge**, because it sat at the same margin as the rows and nothing said you were a level deeper. **`BACKDROP` closes when another fold opens and not on focus loss** — it used to collapse the moment you touched the canvas, which is exactly when its camera calibration is being used.

- **The video frame was seeded into `ALWAYS` and belongs in `Song with video and lyrics`** (v1.21.0, 2026-09-05, Jorge). **The ghost it caused:** `SONG VIDEO / Video frame` painted on the wall while `Song with lyrics` was previewed. An always-on shape is always on — **the room was right and the seed was wrong.** `ALWAYS` is empty in the seeded room now, which is correct rather than an omission: nothing in the designed default is meant to be up in both branches. **The round before this one disproved the literal report — *`Video lyrics` arrives in `ALWAYS`* — three ways, correctly, and stopped.** The symptom was one row away and survived the disproof. **Disproving a claim is not explaining a symptom**, and a report names a symptom.
- **`SCOPE` and `PREVIEWING` moved above the canvas, and `PREVIEWING` is a dropdown** (v1.21.0). They are the two controls that decide what you are looking at, and they sat one in the sidebar and one over it. **This supersedes the two side-by-side green/grey buttons** asked for the day before — a reversal, not a misreading: two controls answering one kind of question should read as one kind of thing.
- **A live output window stopped following, and the cause was the deduplication** (v1.21.0). Re-entering the gig showed the logo in the preview and not on the wall — the window was painting the room as it was when the gig was created. **`seq` deduplicates double delivery, but `outboundSeq` restarts at 0 on every page load while the window's high-water mark does not**, so a window that had seen `seq 40` discarded every message a freshly built frame would ever send. **Reproduced before it was fixed.** The stream is identified now, not just the message. **And Jorge's own ask is built too** — the output window closes when the visuals step is left — because closing it is the workaround and the staleness was the defect.
- **Media stays inside the visuals folder: no copy, no reference** (v1.21.0, Jorge, after both alternatives were put to him and rejected). A file chosen outside it is **refused**, and moving it in is his to do. **It is not an exception to the boundary rule, it is the rule stated properly** — *visuals is used to read, not to write*: **the tools own a room where they write and own nothing where they only read.** The `Choose from the visuals folder` dropdown came out; **the picker is the only control**, it opens in the visuals folder, and **after a refusal the shape stays empty** so a shape pointing outside that folder never exists at all. **Hosted the host picks**, because a cross-origin frame cannot open a picker and an `<input type=file>` hands over a bare name with no path — which cannot tell a `logo.png` in the folder from one on the Desktop. **A false accept is worse than a false reject.**
- **A file copied into the visuals folder by hand was still reported missing** (v1.21.0). The listing and the resolved bytes were only ever recomputed when this tool did something, and **copying a file in is not something this tool does.** There is a `Look again` on the complaint itself, and the folder is re-read when the window comes back.
- **`BACKDROP` never collapsed — the previous fix overshot** (v1.21.0). Closing it only when another fold opened was too little: the sidebar has one other fold and it is usually hidden. **It closes when a pointer lands anywhere in the side menu outside it**, and survives a canvas click, a recalibration and dragging the calibration points — which is the whole ask.
- **A video assigned to a song was silently not saved** (v1.22.0, 2026-09-06, found at a wall). Assigned for `tragedia`, walked through to sign-off, and Pregonero's `video` drive mode was then unavailable **because the song had no video**. The gig's `visuals.json` had `songVisuals.assets: {}` with `defaults` beside it fully populated — **the file was written and the assignment was not in it.** Nothing failed and nothing warned. **The cause is a rule that outlived its listing:** `sanitizeSongAssets` refused any value carrying a separator, because *a path here would be a fact about one machine written into a file built to travel* — and on 2026-09-04 the asset listing was made to **recurse**, on the argument that the constraint that mattered was *not absolute*, never *one segment*, and because the real visuals folder keeps **every animation one level down in a per-song directory**. The picker changed; the refusal did not, so from that day **every assignment made in that folder was dropped on the way to disk.** `mapper/assetName.js` is the rule now — absolute, escaping, a backslash, an empty or `.`/`..` segment, or deeper than the walk goes — with its own `node --test` suite in CI. **It is a cross-repo contract**: Pregonero's `visualsFile.assetNameIsRelative` is the same function against the same cases, because a value one side writes and the other refuses is the mismatch these two keep meeting.
- **The room's file has a format name: `RP JSON` — Room Performance** (v1.23.0, 2026-09-06, Jorge). It joins `SP JSON` (Song Performance, Bombista's) and `GP JSON` (Gig Performance, Tramoya's). **The FILENAME is not changing** — `visuals.json` stays, because a format name and a filename are different things and this one is load-bearing in code that compares strings. *Room* rather than *visuals* because that is already the vocabulary here — *map the room*, *the room's shapes* — and because the file holds a calibration and a set of shapes, which is a room and not a picture. The canonical description of all three lives in the tramoya-integration vault at `docs/formats.md`, which supersedes `gig-file.md`; the pointers in this repo follow it.

Three test suites, covering three files. `mapper/warp.test.mjs` pins the warp, because that file is shared with another repo and drift between the two would show up as a few pixels of rotation on a wall in front of people. `mapper/stageCapture.test.mjs` pins the stage capture's transform, because a capture taken the wrong way through the calibration **still looks like the stage** and puts every shape drawn on it in the wrong place at the venue. `mapper/modes.test.mjs` pins the mode resolution, including a hand-written three-mode room, because the file format claims a list and a claim nobody exercises is the one that turns out to be false at a wall. The rest is still a spike, run with spike discipline, and verified the way the notes above describe: in real headed Chrome, against what is painted.
...
There were four. `song-intro` — the title card — and `gig-contact` — the line and the QR at the end of the night — **stopped being shapes on 2026-09-04**. Pregonero draws both into a shape that is already there, the video frame or the lyrics shape, and decides which. **Muralista owns where things are; Pregonero owns what is showing when**, and an intro is a *when*: it is up before the cue and gone after it. On the wall the two of them landed wherever there was room left — the intro on a whiteboard, the contact on bare planks — because a shape that is up only when nothing else is had to claim its own territory. It needs none.
