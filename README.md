...
One test suite, covering one file. `mapper/warp.test.mjs` pins the warp, because that file is shared with another repo and drift between the two would show up as a few pixels of rotation on a wall in front of people. The rest is still a spike, run with spike discipline, and verified the way the notes above describe: in real headed Chrome, against what is painted.
