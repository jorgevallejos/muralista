...
5. ~~Integration with the lyric translator's clock/timeline (shared transport)~~ — **superseded
   ...
   Pregonero reads and executes it; Pregonero owns the only clock on stage. See
   `muralista-v1-design.md` §3.
...
- **2026-08-20: Muralista v1 design session (Cowork).** Supersedes the "parked until Q4 animation project is defined" framing above (staged-plan step 5-adjacent) — that blocker no longer gates the project. New goal, in Jorge's words: a first version usable in small-venue concerts, where the projector floods the *whole* background including the performer, and the mapping decides what goes where — including what stays dark. Full design: `muralista-v1-design.md` (this folder).
  - **Architecture decided:** Muralista becomes a desk/authoring tool, like Bombista — it prepares a venue configuration file at the venue, before the show. **Pregonero reads and executes that file on stage**; Muralista does not run during the show. One renderer, one clock (Pregonero's). See design doc §3.
  - **Frame change:** additive (add quads; everything outside is black by default) → subtractive (flood the whole field with the projector; decide what stays dark). Negative space is now a first-class primitive; **keep-out regions are the new object** (design doc §2).
  - **Decided: a static performer keep-out box for v1, not camera tracking.** Deterministic, rehearsable, works tonight, needs no camera. Camera silhouette tracking (the Facecam's eventual job) is explicitly out of v1 — a flickering mask in low light mid-song is worse than none. Design doc §2.1.
  - **Debt flagged:** the v2 slices below (`976dd84`, `0d633cb`, `065c03f`, 2026-07-02/03) are still untested by hand — seven weeks headless-only. And **the runtime half of that v2 work is now on the wrong side of the tool boundary** it built for a Muralista that runs during the show; under the new architecture it doesn't. That work is relocated, not wasted (it becomes properties declared in the venue file and executed by Pregonero) but porting it is real, uncosted work. Design doc §3.2.
  - **Next step (design doc §8):** a projector session that does two things at once — clears the seven-week-old untested v2 by hand, and fakes the new subtractive layout by hand (place a region where lyrics would go, place a black quad where the body is, look at it). Bring back a hand-written notes file — the venue file's first draft.
...
- **Next (superseded 2026-08-20 — see Status/next step above for the current plan):** Jorge tests v2 (desk first — mic checks in README's Sound-reactivity section — then projector with overlay + mic beside cerdo); then the asset-bank design conversation; then decide GitHub/PR graduation. Kept as history — the desk/projector testing of v2 is still owed, it is now folded into the 2026-08-20 projector session (design doc §8) rather than a standalone step.
- **Next, as of 2026-08-20:** a projector session with the rig as it stands — clear the untested v2, and by hand fake the subtractive layout (lyric region, performer keep-out) described in `muralista-v1-design.md` §8. Do not build before that session.
